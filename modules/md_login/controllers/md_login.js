...
exports.index = function(req, res){
	...
	res.render('../modules/md_login/views/vw_login.html', res.pageInfo);    
...
exports.process = function(req,res){
	...
	if(req.param('email') != ''  && req.param('password') != '')
    {
		...
		db.get().collection('users',function(err,collection){
			...
			if(passwordData.passwordHash == storedpass)
			{
               ...
               // });
                ...
                collection.update({'email' : email},item,{safe:true}, function(err, result) {
                    if (err) {
                        res.send(200,{ success : false, message : 'error 501'});
                    } else {
                        res.send(200,{ success : true, message : 'Success 200', 
							//token : token
							});
                    }
                });
			...
			{
				res.send(200,{ success : false, message : 'error 401' });
			}
        }
        else
        {
		...
		{
			 res.send(404,{ success : false, message : 'error 404 2' });
		}
		})
		})
	}
